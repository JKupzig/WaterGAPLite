...
wb <- runModel(x$SimPeriod, x, Settings, 5)
df <- data.frame("Date"= x[["SimPeriod"]], "Sim_mm"=wb$routing$River$Discharge)
df$Sim = Q.convert_mmday_m3s(df$Sim_mm, sum(x$GAREA))
Qobs <- Q.readGRDC(x$id, NA, NA,  
                   min(x[["SimPeriod"]]), max(x[["SimPeriod"]]), 
