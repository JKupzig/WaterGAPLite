...
init.settings <- function(water_use="off",
  ...
  water_use_setting <- switch(water_use, "off"=0, "on"=1, "including cities"=2)
  if (is.null(water_use_setting)) {
    stop(sprintf("water use setting '%s' not valid choose between 'off', 'on', 'including cities'", water_use))
  }
  
  water_use_allocation_setting <- switch(water_use_allocation, "temporal and spatial"=0, "spatial"=1, "temporal"=2)
  ...
  flow_velocity_setting <- switch(flow_velocity, "constant"=0, "variable"=1)
  ...
  gap_year_setting <- switch(gap_year, "included"=0, "not included"=1)
  if (is.null(gap_year_setting)) {
    stop(sprintf("gap year setting '%s' not valid choose between 'included', 'not included'", gap_year))
  }
  
  reservoir_algorithm_setting <- switch(reservoir_algorithm, "Hanasaki"=0, "as lakes"=1)
  ...
  use_splitting_factor_setting <- switch(use_splitting_factor, "off"=0, "on"=1)
  ...
  longwave_radiation_setting <- switch(longwave_radiation, "reading"=0, "calculation"=1)
  ...
  initial_states_setting <- switch(initial_states, "not known"=0, "reading"=1, "writing"=2, "reading and writing"=3)
  ...
  snow_in_wetland_setting <- switch(snow_in_wetlands, "off"=0, "on"=1)
