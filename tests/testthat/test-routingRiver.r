...
testthat::test_that("routing river (old)",
{
  # note that the two vectors are modified by reference, so
  # they must be defined in every function call from scratch
  # thus, not variable can be used as for cell, velocity_in_d and inflow!
  
  # Das Problem mit dieser Funktion ist, dass der aktuelle Zustand nicht 
  # vom vorherigen Zeitschritt beeinflusst ist! Damit erzielt bspw. 
  # evaporation from river keinen Effekt und führt zu einer Differenz
  # in der Wasserbilanz!
  ...
  routed_water <- WaterGAPLite:::routingRiverOld(
    cell, velocity_in_d, inflow, c(1,1,1,1), c(0,0,0,0))

  routed_water_slow <- WaterGAPLite:::routingRiverOld(
    cell, velocity_in_d + 0.1, inflow, c(1,1,1,1), c(0,0,0,0))

  routed_water_more_in <- WaterGAPLite:::routingRiverOld(
    cell, velocity_in_d, inflow + 0.1, c(1,1,1,1), c(0,0,0,0))

  routed_water_more_prestep <- WaterGAPLite:::routingRiverOld(
    cell, velocity_in_d, inflow, c(1,1,1,1) + c(0,0.1,0,0), c(0,0,0,0))

  routed_water_more_storage <- WaterGAPLite:::routingRiverOld(
    cell, velocity_in_d, inflow, c(1,1,1,1), c(0,0,0,0)*2)

  expect_equal(routed_water, 0.42612264) # statt: 1.39346934
  expect_gt(routed_water, routed_water_slow)
  expect_gt(routed_water_more_in, routed_water)
  expect_equal(routed_water_more_storage, routed_water) # problem!
  expect_equal(routed_water, routed_water_more_prestep) #
