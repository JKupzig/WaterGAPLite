	...
	for (int time = 0; time < ndays; time++){
		...
		for (int n = 0; n < numbers.length(); n++){
			...
			for (int i=0; i < cellIDs.length(); i++) {
				...
				double calculated_duration = G_riverLength[cell] / riverVelocity;
				...
				{
					RoutedOutflowCell = routingRiver(
						cell,
						calculated_duration,
						river_in,
						QA_river,
