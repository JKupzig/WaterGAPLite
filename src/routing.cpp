			 ...
			 NumericMatrix PETw, NumericMatrix Prec)
{
	...
	// Compute forecasts for Schneider's dam algorithm
	...
	if (ReservoirType == 2)
	{
		for (int cell = 0; cell < array_size; cell++)
		{
			if (G_RESAREA[cell] > 0)
			{
				for (int day_counter = 0; day_counter < ndays; day_counter++)
				{
					Date SimDate = SimPeriod[day_counter];
	...
	List Res = List::create(Named("Overflow") = OverflowRes, Named("Outflow") = OutflowRes, Named("Evapo") = EvapoRes, Named("Storage") = StorageRes, Named("Inflow") = InflowRes, Named("Target") = targetRes);
