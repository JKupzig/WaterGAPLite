...
List routing(DateVector SimPeriod, NumericMatrix surfaceRunoff, NumericMatrix GroundwaterRunoff,
	...
	for (int day = 0; day < ndays; day++){
		...
		for (int n = 0; n < numbers.length(); n++){
			...
			for (int i=0; i < cellIDs.length(); i++) {
				...
				// local lakes
				if (G_LOCLAK[cell] > 0) {
					out_loclake = routingLocalWaterBodies(0, cell, PrecWater, PETWater, TempWater, accumulated_days_below_x, snow_storage_for_wetland, LandInflow,
				...
				//local wetlands
				if (G_LOCWET[cell] > 0) {
					out_locwet = routingLocalWaterBodies(1, cell, PrecWater, PETWater,  TempWater, accumulated_days_below_x, snow_storage_for_wetland, out_loclake,
								S_locLakeStorage, locLake_overflow, locLake_outflow, locLake_evapo, locLake_inflow,
	...
	List locWetland =List::create(Named("Overflow") = OverflowlocWetland, Named("Outflow") = OutflowlocWetland, Named("Evapo") = EvapolocWetland, Named("Storage") = StoragelocWetland, Named("Inflow") = InflowlocWetland, Named("Snow") = snow_local_wetland);
